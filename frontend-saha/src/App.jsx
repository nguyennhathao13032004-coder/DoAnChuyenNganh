...
import { 
  ...
  Facebook, Instagram, Youtube, Clock, LogOut
} from 'lucide-react';
...
const App = () => {
  ...
  // 1. Kéo dữ liệu giỏ hàng ra
  const { cartCount } = useCart();
  
  // 2. Biến để điều khiển hiệu ứng nảy nảy
  const [isAnimating, setIsAnimating] = useState(false);

  // 3. Mỗi khi số lượng (cartCount) thay đổi -> Bật hiệu ứng 300ms rồi tắt
  useEffect(() => {
    if (cartCount > 0) {
      setIsAnimating(true);
  ...
  // ==========================================
  // KHAI BÁO CÁC STATE (BIẾN CHỨA DỮ LIỆU)
  // ==========================================
  const [user, setUser] = useState(null);
  const [realProducts, setRealProducts] = useState([]);
  const [blogs, setBlogs] = useState([]); // <-- Đây là biến Hào bị thiếu lúc nãy
  const [searchQuery, setSearchQuery] = useState('');
  // ==========================================
  // GỌI API LẤY DỮ LIỆU KHI TRANG VỪA TẢI
  // ==========================================
  useEffect(() => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {mockCategories.map(cat => (
            <div 
              ...
              className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 text-center cursor-pointer hover:border-brand/50 hover:shadow-md transition-all"
            >
